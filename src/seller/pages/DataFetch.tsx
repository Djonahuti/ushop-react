...
export const DataFetch = () => {
  ...
  useEffect(() => {
    const fetchSellerId = async () => {
      ...
      try {
        const sellers = await apiGet<Array<{ seller_id: number }>>(`/sellers.php?email=${encodeURIComponent(email)}`);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const productsData = await apiGet<Product[]>(`/products.php?seller_id=${sellerId}`);
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const hydrated = await Promise.all((ordersData || []).map(async (order) => {
          ...
          const allProducts = await apiGet<Array<{ product_id: number; seller_id: number; product_title: string; product_price: number; product_img1: string }>>('/products.php');
          ...
          const pending_order_items = sellerItems.map(item => ({
            ...item,
            products: allProducts?.find(p => p.product_id === item.product_id),
          }));
        ...
        setOrders(hydrated.filter(o => o !== null) as PendingOrder[]);
      } catch (err) {
