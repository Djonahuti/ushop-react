...
try {
    // Get order and customer details before update
    $stmt = $pdo->prepare('SELECT o.*, c.customer_name, c.customer_email FROM public.orders o LEFT JOIN public.customers c ON o.customer_id = c.customer_id WHERE o.invoice_no = :inv');
    ...
    // Update order status
    $stmt = $pdo->prepare('UPDATE public.orders SET order_status = :s WHERE invoice_no = :inv');
